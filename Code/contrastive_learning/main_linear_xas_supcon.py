...
def parse_option():
    ...
    safe_ckpt = opt.ckpt.replace('/', '_')

    opt.model_name = 'XASSupCon__{}_{}_lr_{}_decay_{}_bsz_{}_size_{}'.\
        format(opt.dataset, opt.model, opt.learning_rate, opt.weight_decay,
               opt.batch_size, opt.size)

    # opt.model_name = 'SupCon_Linear_{}_{}_lr_{}_decay_{}_bsz_{}'.\
...
def train(train_loader, model, classifier, criterion, optimizer, epoch, opt):
    ...
    for idx, (images, labels) in enumerate(train_loader):
        ...
        # print('top1', top1.val.item())
        # print('top1', top1.avg.item())
        # print('loss', losses.val)
        # SGD
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
